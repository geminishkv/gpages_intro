export const LOGO_IMG  = process.env.PUBLIC_URL + '/img/logotype/logo_white.svg';
export const UWU_IMG   = process.env.PUBLIC_URL + '/img/hero/uwu.webp';
export const LIDER_IMG = process.env.PUBLIC_URL + '/img/badges/lider.png';
export const SBERSPASIBO_IMG = process.env.PUBLIC_URL + '/img/badges/sberspasibo.png';
export const BMSTU_IMG = process.env.PUBLIC_URL + '/img/badges/bmstu.png';
export const SUSU_IMG = process.env.PUBLIC_URL + '/img/badges/susu.png';
export const SPECIALIST_IMG = process.env.PUBLIC_URL + '/img/badges/specialist.svg';
export const MPFI_IMG  = process.env.PUBLIC_URL + '/img/badges/mpfi.png';
export const RBPO_IMG   = process.env.PUBLIC_URL + '/img/badges/rbpo.png';
export const INSECA_IMG = process.env.PUBLIC_URL + '/img/badges/inseca.png';

export const RESUME_URL = 'https://hh.ru/resume/af4cc9ceff086141d00039ed1f4b4a6c35706f';

export const TITLE_TEXT    = 'geminishkv';
export const SUBTITLE_TEXT = 'AppSec & DevSecOps';
export const TAGLINE_TEXT  = 'Sic Parvis Magna. Auxilio Divino';

