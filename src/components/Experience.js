...
function Node({ item }) {
  const logos = item.logos ?? [{ src: item.logo, alt: item.company, url: item.url, round: item.logoRound, inset: item.logoInset }];
  ...
  return (
    <div className={cls}>
      {logos.map((l, j) => {
        ...
        const imgCls = `pipe__logo${l.round ? ' pipe__logo--round' : ''}${l.inset ? ' pipe__logo--inset' : ''}`;
        return l.url ? (
